...
{
    public partial class WERD0311 : WebUIBasePage
    {
        ...
        DataTable dt1
        {
            get
            {
                return (DataTable)Session["WERD0311"];
            }
            set
            {
                Session["WERD0311"] = value;
            }
        }
        ...
        void ColumnsClear(int nSchGbn)
        {
            ...
            string[][] gridColumn = new string[][] { 
                    new string[]{"F_ORDER","F_STEP","F_WORKCD","F_WORKNM","F_TITLE"},
                    new string[]{"F_ORDER","F_STEP","F_WORKCD","F_WORKNM","F_ITEMCD","F_ITEMNM","F_TITLE"}
                };
            ...
            foreach (string col in gridColumn[nSchGbn])
            {
                ...
                if (!"F_STEP,F_WORKCD,F_ITEMCD".Contains(column.FieldName))
                {
            ...
            if (nSchGbn == 0)
            {
                new ASPxGridViewCellMerger(grid, "F_WORKNM");
            }
            else
            {
                new ASPxGridViewCellMerger(grid, "F_WORKNM|F_ITEMCD|F_ITEMNM");
            }
        }
        ...
        void WERD0311_LST(int nSchGbn, bool bPrint = false)
        {
            ...
            using (WERDBiz biz = new WERDBiz())
            {
                ...
                if (nSchGbn == 0)
                {
                    ds = biz.WERD0311_MODEL_LST(oParamDic, out errMsg);
                }
                else
                {
                    ds = biz.WERD0311_ITEM_LST(oParamDic, out errMsg);
                }
            }
            dt1 = ReCreateColumns(nSchGbn, grid, setGrid(nSchGbn));
            grid.DataSource = dt1;
                ...
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    if (i == 0)
                    {
                        ...
                        for (int z = 0; z < 6; z++)
                        {
                            ...
                            NewDt.Rows[z]["F_WORKNM"] = ds.Tables[0].Rows[i]["F_WORKNM"].ToString();
                        }
                    ...
                    {
                        string strNextItem = strNextItem = NewDt.Rows[NewDt.Rows.Count - 1]["F_WORKNM"].ToString();
                        string strItemcd = ds.Tables[0].Rows[i]["F_WORKNM"].ToString();
                        ...
                        NewDt.Rows[NewDt.Rows.Count - 6]["F_WORKNM"] = ds.Tables[0].Rows[i]["F_WORKNM"].ToString();
                        NewDt.Rows[NewDt.Rows.Count - 5]["F_WORKNM"] = ds.Tables[0].Rows[i]["F_WORKNM"].ToString();
                        NewDt.Rows[NewDt.Rows.Count - 4]["F_WORKNM"] = ds.Tables[0].Rows[i]["F_WORKNM"].ToString();
                        NewDt.Rows[NewDt.Rows.Count - 3]["F_WORKNM"] = ds.Tables[0].Rows[i]["F_WORKNM"].ToString();
                        NewDt.Rows[NewDt.Rows.Count - 2]["F_WORKNM"] = ds.Tables[0].Rows[i]["F_WORKNM"].ToString();
                        NewDt.Rows[NewDt.Rows.Count - 1]["F_WORKNM"] = ds.Tables[0].Rows[i]["F_WORKNM"].ToString();
            ...
            else
            {
                ...
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    else
                    {
                        ...
                        NewDt.Rows[NewDt.Rows.Count - 4][ds.Tables[0].Rows[i]["F_WORKDATE"].ToString()] = Convert.ToInt32(ds.Tables[0].Rows[i]["F_NGQTY"]).ToString("#,##0");
                        NewDt.Rows[NewDt.Rows.Count - 3][ds.Tables[0].Rows[i]["F_WORKDATE"].ToString()] = ds.Tables[0].Rows[i]["F_NGRATE"].ToString();
                        NewDt.Rows[NewDt.Rows.Count - 2][ds.Tables[0].Rows[i]["F_WORKDATE"].ToString()] = Convert.ToInt32(ds.Tables[0].Rows[i]["F_LOSSAMT"]).ToString("#,##0");
                        NewDt.Rows[NewDt.Rows.Count - 1][ds.Tables[0].Rows[i]["F_WORKDATE"].ToString()] = Convert.ToInt32(ds.Tables[0].Rows[i]["F_LOSSAMT"]).ToString("#,##0");
                    }
                }
            }
        ...
        protected void devGrid_CustomSummaryCalculate(object sender, DevExpress.Data.CustomSummaryEventArgs e)
        {
            ...
            {
                if ((grid.ID == "devGrid1" && item.FieldName == "F_WORKNM")
            ...
            {
                if (!"F_STEP,F_ITEMCD,F_WORKCD,F_WORKNM,F_ITEMNM,F_TITLE".Contains(item.FieldName))
                {
            ...
            if (e.SummaryProcess == DevExpress.Data.CustomSummaryProcess.Finalize)
            {
                if (grid.VisibleRowCount > 0)
                {
                   ...
                   || (grid.ID == "devGrid2" && item.FieldName == "F_ITEMNM"))
                    ...
                    if (!"F_STEP,F_WORKCD,F_WORKNM,F_ITEMCD,F_ITEMNM,F_TITLE".Contains(item.FieldName))
                    {
        ...
        protected void devGrid_CustomColumnDisplayText(object sender, ASPxGridViewColumnDisplayTextEventArgs e)
        {
            ...
            if ("F_ORDER,F_STEP,F_WORKCD,F_WORKNM,F_ITEMCD,F_ITEMNM,F_TITLE".Contains(fieldName))
                return;
