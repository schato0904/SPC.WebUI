...
{
    public partial class WERD0207 : WebUIBasePage
    {
        ...
        void WERD0207LST()
        {
            ...
            using (WERDBiz biz = new WERDBiz())
            {
                ...
                ds = biz.WERD0207_LST(oParamDic, out errMsg);
            }
        ...
        void SetGrid(DataTable dt, DataTable dt2, DataTable dt3)
        {
            ...
            foreach (DataRow dtRow1 in dt.Rows)
            {
                ...
                dtNewRow["검사일자"] = string.Format("{0:n0}",(dtRow1["F_WORKDATE"] ?? "").ToString());
                dtNewRow["생산수량"] = string.Format("{0:n0}", Convert.ToInt32(dtRow1["F_PRODUCTQTY"] ?? "").ToString("#,##0"));
                dtNewRow["검사수량"] = string.Format("{0:n0}", Convert.ToInt32(dtRow1["F_INSPQTY"] ?? "").ToString("#,##0"));
                dtNewRow["부적합수량"] = string.Format("{0:n0}", Convert.ToInt32(dtRow1["F_NGQTY"] ?? "").ToString("#,##0"));
        ...
        {
            WERD0207LST();
            devGridExporter.WriteXlsToResponse(String.Format("일별부적합현황_{0}", DateTime.Today.ToShortDateString()), new DevExpress.XtraPrinting.XlsExportOptionsEx { ExportType = DevExpress.Export.ExportType.WYSIWYG });
        }
        #endregion
