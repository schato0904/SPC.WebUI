...
{
    public partial class PFRC0206 : WebUIBasePage
    {
        ...
        void QCDMONITORING_LST()
        {
            ...
            using (PFRCBiz biz = new PFRCBiz())
            {
                ...
                ds = biz.QCDMONITORING_LST(oParamDic, out errMsg);
            }
        ...
        protected void devGrid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (e.UpdateValues.Count > 0)
            {
                foreach (var Value in e.UpdateValues)
                {
                    ...
                    oParamDic.Add("F_COMPCD", Value.Keys["F_COMPCD"].ToString());
                    oParamDic.Add("F_FACTCD", Value.Keys["F_FACTCD"].ToString());
                    oParamDic.Add("F_BANCD", Value.Keys["F_BANCD"].ToString());
                    oParamDic.Add("F_LINECD", Value.Keys["F_LINECD"].ToString());
                    oParamDic.Add("F_STATUS", Value.NewValues["F_STATUS"].ToString().ToLower().Equals("true") ? "1" : "0");
                    oParamDic.Add("F_USER", gsUSERID);

                    oSPs.Add("USP_QCDMONITORING_INS_UPD");
            ...
            using (PFRCBiz biz = new PFRCBiz())
            {
                bExecute = biz.PROC_QCDMONITORING_BATCH(oSPs.ToArray(), oParameters.ToArray(), out resultMsg);
            }
