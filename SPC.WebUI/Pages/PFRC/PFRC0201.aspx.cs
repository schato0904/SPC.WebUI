...
{
    public partial class PFRC0201 : WebUIBasePage
    {
        ...
        {
            DevExpressLib.GetUsedString(new string[] { "F_STATUS", "F_TOOLBARR", "F_TOOLBARC", "F_TOOLBARU", "F_TOOLBARW", "F_TOOLBARS", "F_TOOLBARD", "F_TOOLBARP", "F_TOOLBARE" }, e);
        }
        #endregion
        ...
        protected void devGrid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            StringBuilder sb_ToolBars = new StringBuilder();
            string[] arrToolBars = { "R", "C", "U", "W", "S", "D", "P", "E" };
            ...
            if (e.InsertValues.Count > 0)
            {
                ...
                {
                    sb_ToolBars = new StringBuilder();
                    oParamDic = new Dictionary<string, string>();
                    oParamDic.Add("F_PGMID", (Value.NewValues["F_PGMID"] ?? "").ToString());
                    oParamDic.Add("F_PGMNMKR", (Value.NewValues["F_PGMNMKR"] ?? "").ToString());
                    oParamDic.Add("F_PGMNMUS", (Value.NewValues["F_PGMNMUS"] ?? "").ToString());
                    oParamDic.Add("F_PGMNMCN", (Value.NewValues["F_PGMNMCN"] ?? "").ToString());
                    oParamDic.Add("F_LINK", (Value.NewValues["F_LINK"] ?? "").ToString());
                    oParamDic.Add("F_MENULEVEL", (Value.NewValues["F_MENULEVEL"] ?? "").ToString());
                    oParamDic.Add("F_STATUS", (Value.NewValues["F_STATUS"] ?? "").ToString());
                    foreach (string ToolBar in arrToolBars)
                    {
                        sb_ToolBars.Append(!(bool)Value.NewValues["F_TOOLBAR" + ToolBar] ? "" : ToolBar);
                    }
                    oParamDic.Add("F_TOOLBAR", sb_ToolBars.ToString());
                    oParamDic.Add("F_USER", gsUSERID);

                    oSPs[idx] = "USP_SYPGM01_INS";
            ...
            if (e.UpdateValues.Count > 0)
            {
                foreach (var Value in e.UpdateValues)
                {
                    ...
                    oParamDic.Add("F_PGMID_NEW", (Value.NewValues["F_PGMID"] ?? "").ToString());
                    oParamDic.Add("F_PGMNMKR", (Value.NewValues["F_PGMNMKR"] ?? "").ToString());
                    oParamDic.Add("F_PGMNMUS", (Value.NewValues["F_PGMNMUS"] ?? "").ToString());
                    oParamDic.Add("F_PGMNMCN", (Value.NewValues["F_PGMNMCN"] ?? "").ToString());
                    oParamDic.Add("F_LINK", (Value.NewValues["F_LINK"] ?? "").ToString());
        ...
        protected void devGrid_InitNewRow(object sender, DevExpress.Web.Data.ASPxDataInitNewRowEventArgs e)
        {
            ...
            foreach (string ToolBar in arrToolBars)
            {
                e.NewValues["F_TOOLBAR" + ToolBar] = false;
            }

            e.NewValues["F_STATUS"] = false;
        }
        #endregion
        ...
        {
            //QWK04A_ADTR0103_LST();
            devGridExporter.WriteXlsToResponse(String.Format("[{0}]{1} 프로그램관리정보", DateTime.Today.ToString("yyyyMMdd"), gsCOMPNM), new DevExpress.XtraPrinting.XlsExportOptionsEx { ExportType = DevExpress.Export.ExportType.WYSIWYG });
        }
        #endregion
