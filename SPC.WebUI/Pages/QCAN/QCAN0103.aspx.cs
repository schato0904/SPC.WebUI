...
{
    public partial class QCAN0103 : WebUIBasePage
    {
        ...
        Int32 QWK04A_ADTR0402_CNT()
        {
            using (QCANBiz biz = new QCANBiz())
            {
                ...
                totalCnt = biz.QWK03A_QCAN0103_CNT(oParamDic);
            }
        ...
        void QWK04A_ADTR0402_MONTH_LST(int nPageSize, int nCurrPage, bool bCallback)
        {
            ...
            using (QCANBiz biz = new QCANBiz())
            {
                ...
                ds2 = biz.QWK03A_QCAN0103_MONTH_LST(oParamDic, out errMsg);
            }
        ...
        void QWK04A_ADTR0402_MONTH_EXCEL()
        {
            string errMsg = String.Empty;
        ...
        void QWK04A_ADTR0402_LST(int nPageSize, int nCurrPage, bool bCallback)
        {
            ...
            using (QCANBiz biz = new QCANBiz())
            {
                ...
                ds = biz.QWK03A_QCAN0103_LST(oParamDic, out errMsg);
            }
            ...
            if (ds2.Tables[0].Rows.Count > 0) {
                ...
                {
                    devGrid.DataColumns["F_DATA" + (i + 1).ToString()].Caption = ds2.Tables[0].Rows[i]["F_YYMM1"].ToString();
                    devGrid.DataColumns["F_DATA" + (i + 1).ToString()].Visible = true;
                }
                for (int i = ds2.Tables[0].Rows.Count; i < 12; i++)
                {
                    devGrid.DataColumns["F_DATA" + (i + 1).ToString()].Visible = false;
                }
            }
        ...
        void QWK04A_ADTR0402_EXCEL()
        {
            ...
            using (QCANBiz biz = new QCANBiz())
            {
                ...
                oParamDic.Add("F_MEANINSPCODE", GetInspItemCD());
                oParamDic.Add("F_CHK", this.rdoHIPISNG.SelectedItem.Value.ToString());
                oParamDic.Add("F_PAGESIZE", QWK04A_ADTR0402_CNT().ToString());
            ...
            if (ds2.Tables[0].Rows.Count > 0)
            {
                for (int i = 0; i < ds2.Tables[0].Rows.Count; i++)
                {
                    devGrid2.DataColumns["F_DATA" + (i + 1).ToString()].Caption = ds2.Tables[0].Rows[i]["F_YYMM1"].ToString();
                    devGrid2.DataColumns["F_DATA" + (i + 1).ToString()].Visible = true;
                }
                for (int i = ds2.Tables[0].Rows.Count; i < 12; i++)
                {
                    devGrid2.DataColumns["F_DATA" + (i + 1).ToString()].Visible = false;
                }
            }
        }
        #endregion
        ...
        protected void devGrid_CustomCallback(object sender, DevExpress.Web.ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            QWK04A_ADTR0402_MONTH_LST(nPageSize, nCurrPage, true);
        ...
        {
            QWK04A_ADTR0402_MONTH_EXCEL();
            QWK04A_ADTR0402_EXCEL();
            devGrid2.DataBind();
            devGridExporter.WriteXlsToResponse(String.Format("[{0}]{1} 공정능력월별정보", DateTime.Today.ToString("yyyyMMdd"), gsCOMPNM), new DevExpress.XtraPrinting.XlsExportOptionsEx { ExportType = DevExpress.Export.ExportType.WYSIWYG });
        }
        #endregion
