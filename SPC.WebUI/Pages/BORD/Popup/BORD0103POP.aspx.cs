...
{
    public partial class BORD0103POP : WebUIBasePage
    {
        ...
        public string[] keyFields = new string[7];
        ...
        void BOARD_DETAIL_LST(){
            ...
            if (keyFields[4] == gsCOMPCD)//현재 접속중인 회사코드와 게시글작성 회사코드 비교
            {
            ...
            else
            {
                using (BORDBiz biz = new BORDBiz())
                {
                    oParamDic.Clear();
                    oParamDic.Add("F_NUMBER", keyFields[0]);
                    oParamDic.Add("F_COMPCD", keyFields[4]);
                    ds = biz.BOARD_REPLY_LST(oParamDic, out errMsg1);
                }
            }
            ...
            {
                this.Page.ClientScript.RegisterStartupScript(this.GetType(), "name", "OptionDisplay();", true);
            }
        }
        #endregion
        ...
        protected void pnlContent_Callback(object sender, DevExpress.Web.CallbackEventArgsBase e)
        {
            ...
            if (Param[0] == "update")
            {
                REPLY_DETAIL_UPD(Param);
            }
            else if (Param[0] == "delete")
            {
                REPLY_DETAIL_DEL(Param);
            }

        }

        void REPLY_DETAIL_UPD(string[] Param)
        {
            ...
            if (bExists)
            {
                using (BORDBiz biz = new BORDBiz())
                {
                    ...
                    oParamDic.Add("F_NUMBER", keyFields[0]);
                    oParamDic.Add("F_COMPCD", keyFields[4]); //작성자가 속한회사
        ...
        void REPLY_DETAIL_DEL(string[] Param)
        {
