...
{
    public partial class PopRptCATM1302 : WebUIBasePage
    {
        ...
        protected void Page_Init(object sender, EventArgs e)
        {
            ...
            //RptCATM1305 report = new RptCATM1305(this.gsCOMPCD, this.gsFACTCD, this.popParam.GetString("F_WORKNO"), this.gsLANGTP);
            string errMsg = string.Empty;
            //var dic = this.GetData(this.popParam.GetString("KEY"), out errMsg);
            //if (!string.IsNullOrWhiteSpace(errMsg)) throw new Exception(errMsg);
            var dic = new Dictionary<string, string>() { { "KEY", this.popParam.GetString("KEY") } };
            string dic2 = this.popParam.GetString("KEY");
            //var dic = new Dictionary<string, string>() { { "F_COMPCD", gsCOMPCD }, { "F_FACTCD", gsFACTCD }, { "F_WORKYMD", this.popParam.GetString("F_WORKYMD") }, { "F_LANGTYPE", gsLANGTP } };

            string[] oParams = new string[7];

            oParams = dic2.Split('|');

            RptCATM1302 report = new RptCATM1302(oParams);
        ...
        /// <returns></returns>
        protected Dictionary<string, string> GetData(string F_WORKYMD, out string errMsg)
        {
            errMsg = string.Empty;
