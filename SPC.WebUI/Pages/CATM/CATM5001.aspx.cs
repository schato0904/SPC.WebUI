...
{
    public partial class CATM5001 : WebUIBasePage
    {
        ...
        {
            get { return Session["CATM5001_Grid"] as DataTable; }
            set { Session["CATM5001_Grid"] = value; }
        }

        // 우측목록
        DataTable CachedData1
        {
            get { return Session["CATM5001_Grid1"] as DataTable; }
            set { Session["CATM5001_Grid1"] = value; }
        }
        #endregion
        ...
        protected void devChart1_CustomCallback(object sender, DevExpress.XtraCharts.Web.CustomCallbackEventArgs e)
        {
            ...
            Series s = new Series("보온로온도", ViewType.Line);
            ...
            s = new Series("분위기온도", ViewType.Line);
            (s.View as LineSeriesView).LineMarkerOptions.Size = 3;
            s.ArgumentDataMember = "F_WORKTIME";
            ...
            s = new Series("보온로온도MAX", ViewType.Line);
            (s.View as LineSeriesView).LineMarkerOptions.Size = 3;
            (s.View as LineSeriesView).Color = System.Drawing.Color.Blue;
            s.ArgumentDataMember = "F_WORKTIME";
            s.ValueDataMembers.AddRange("F_MAX");
            //s.DataSource = dt;
            c.Series.Add(s);

            s = new Series("보온로온도MIN", ViewType.Line);
            (s.View as LineSeriesView).LineMarkerOptions.Size = 3;            
            (s.View as LineSeriesView).Color = System.Drawing.Color.Blue;
            s.ArgumentDataMember = "F_WORKTIME";
            s.ValueDataMembers.AddRange("F_MIN");
            //s.DataSource = dt;
            c.Series.Add(s);

            s = new Series("분위기온도MAX", ViewType.Line);
            (s.View as LineSeriesView).LineMarkerOptions.Size = 3;
            (s.View as LineSeriesView).Color = System.Drawing.Color.Red;
            s.ArgumentDataMember = "F_WORKTIME";
            s.ValueDataMembers.AddRange("F_ATMAX");
            //s.DataSource = dt;
            c.Series.Add(s);

            s = new Series("분위기온도MIN", ViewType.Line);
            (s.View as LineSeriesView).LineMarkerOptions.Size = 3;
            (s.View as LineSeriesView).Color = System.Drawing.Color.Red;
            s.ArgumentDataMember = "F_WORKTIME";
            s.ValueDataMembers.AddRange("F_ATMIN");
            //s.DataSource = dt;
            c.Series.Add(s);
        ...
        void Download_Excelfile()
        {
            ...
            {
                //string file_name = HttpUtility.UrlEncode(string.Format("{0}_{1}.xlsx", "보온로모니터링", DateTime.Now.ToString("yyyyMMddHHmmss")), new System.Text.UTF8Encoding()).Replace("+", "%20");
                string file_name = Uri.EscapeUriString(string.Format("{0}_{1}.xlsx", "보온로 모니터링", DateTime.Now.ToString("yyyyMMddHHmmss")));
        ...
        protected DataTable GetDataRightBody(Dictionary<string, string> dic, out string errMsg)
        {
            ...
            using (CATMBiz biz = new CATMBiz())
            {
                ...
                ds = biz.USP_CATM5001_LST1(oParamDic, out errMsg);
            }
