...
{
    public partial class CATM1303 : WebUIBasePage
    {
        ...
        {
            get { return Session["CATM1303_Grid"] as DataTable; }
            set { Session["CATM1303_Grid"] = value; }
        }

        // 우측목록
        DataTable CachedData1
        {
            get { return Session["CATM1303_Grid1"] as DataTable; }
            set { Session["CATM1303_Grid1"] = value; }
        }
        #endregion
        ...
        protected void devGrid1_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            if (e.Parameters == "GET")
            {
                ...
                DataSet ds = this.GetDataRightBody(p1, out errMsg);

                if (ds == null || ds.Tables.Count < 2) return;

                this.CachedData1 = this.AutoNumberTable(ds.Tables[0].Copy());
                this.CachedData = ds.Tables[1].Copy();
        ...
        {
            var g = devGrid1 as ASPxGridView;
            g.Columns.Clear();
            g.Columns.Add(new GridViewDataColumn("No", "No.") { Width = Unit.Pixel(60) });
            g.Columns.Add(new GridViewDataColumn("F_PLANYMD", "작업지시일") { Width = Unit.Pixel(120) });
            g.Columns.Add(new GridViewDataColumn("F_MACHNM", "설비") { Width = Unit.Pixel(120) });
            g.Columns.Add(new GridViewDataColumn("F_WORKNO", "작지번호") { Width = Unit.Pixel(120) });
            g.Columns.Add(new GridViewDataColumn("F_ITEMCD", "품번") { Width = Unit.Pixel(120) });
            g.Columns.Add(new GridViewDataColumn("F_SDATETIME", "작업시작시간") { Width = Unit.Pixel(120) });
            g.Columns.Add(new GridViewDataColumn("F_EDATETIME", "작업완료시간") { Width = Unit.Pixel(120) });
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                if (dt1.Columns.Contains(cd))
                {
                    g.Columns.Add(new GridViewDataColumn(cd, nm) { Width = Unit.Pixel(120) });
                }
            }
        }
        #endregion
        ...
        /// <returns></returns>
        protected DataSet GetDataRightBody(Dictionary<string, string> dic, out string errMsg)
        {
            DataSet ds = null;
            ...
            using (CATMBiz biz = new CATMBiz())
            {
                ...
                ds = biz.USP_CATM1303_LST1(oParamDic, out errMsg);
            }
