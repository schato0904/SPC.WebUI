...
namespace SPC.WebUI.Pages.MEAS
{
    ...
    /// 공통코드관리(계측기)
    /// </summary>
    public partial class MEAS9001 : WebUIBasePage
    {
        ...
        {
            // 자료유형 목록 조회
            GetCommonCodeList("SS", cboF_CODEGROUP);
            cboF_CODEGROUP.DataBind();
            cboF_CODEGROUP.SelectedIndex = 0;
        }
        #endregion
        ...
        {
            SetGridBind((this.cboF_CODEGROUP.Value ?? "").ToString());
        ...
        void SetGridBind(string COMMCD)
        {
            string errMsg = String.Empty;
        ...
        protected void devGrid_InitNewRow(object sender, DevExpress.Web.Data.ASPxDataInitNewRowEventArgs e)
        {
            ...
            txtGROUPCD.Text = (cboF_CODEGROUP.Value ?? "").ToString();
            ...
            // 코드 기본값 세팅
            var txtCOMMCD = DevExpressLib.devTextBox(devGrid, "txtCOMMCD");
            DataSet ds = devGrid.DataSource as DataSet;
            DataTable dt = (ds != null && ds.Tables.Count > 0) ? ds.Tables[0] : null;
            var maxcommcd = (dt.Rows.Count > 0) ? (dt.Compute("MAX(F_COMMCD)", "") ?? "").ToString() : "";
            int nextNo = int.TryParse((string.IsNullOrWhiteSpace(maxcommcd)) ? "0" : maxcommcd.Trim().Substring(maxcommcd.Length - 2, 2), out nextNo) ? nextNo + 1 : 1;
            var defaultCommcdValue = txtGROUPCD.Text + ((nextNo + 100)%100).ToString("00");
            txtCOMMCD.Text = defaultCommcdValue;

            // 출력순번 기본값 세팅
            var txtSORTNO = DevExpressLib.devSpinEdit(devGrid, "txtSORTNO");
            var maxSortNo = (dt.Rows.Count > 0) ? (dt.Compute("MAX(F_SORTNO)", "") ?? "").ToString() : "";
            int nextSortNo = int.TryParse(maxSortNo, out nextSortNo) ? nextSortNo + 1 : 0;
            txtSORTNO.Value = nextSortNo;
        }
        #endregion
        ...
        protected void devGrid_RowInserting(object sender, DevExpress.Web.Data.ASPxDataInsertingEventArgs e)
        {
            ...
            // 그리드정보를 구한다
            SetGridBind(oParamDic["F_GROUPCD"]);
        }
        #endregion
