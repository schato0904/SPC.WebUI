...
{
    public partial class BSIF0206 : WebUIBasePage
    {
        ...
        DataSet QCD73_LST(DevExpress.Web.GridViewDataComboBoxColumn comboBox)
        {
            string errMsg = String.Empty;
        ...
        DataSet QCE33_LST()
        {
            ...
            using (BSIFBiz biz = new BSIFBiz())
            {
                ...
                ds = biz.QCE33_LST(oParamDic, out errMsg);
            }
        ...
        void QCE34_LST()
        {
            ...
            using (BSIFBiz biz = new BSIFBiz())
            {
                ...
                ds = biz.QCE34_LST(oParamDic, out errMsg);
            }
        ...
        protected void devGrid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (e.InsertValues.Count > 0)
            {
                foreach (var Value in e.InsertValues)
                {
                    ...
                    oParamDic.Add("F_WORKCD", (Value.NewValues["F_BANCD"] ?? "").ToString() + (Value.NewValues["F_LINECD"] ?? "").ToString());
                    oParamDic.Add("F_NGCODE", (Value.NewValues["F_NGCODE"] ?? "").ToString());
            ...
            //        oParamDic.Add("F_WORKCD", (Value.Values["F_BANCD"] ?? "").ToString() + (Value.Values["F_LINECD"] ?? "").ToString());
            //        oParamDic.Add("F_NGCODE", (Value.Values["F_NGCODE"] ?? "").ToString());
            //        oParamDic.Add("F_SORTNO", (Value.Values["F_SORTNO"] ?? "").ToString());
            //        oParamDic.Add("F_STATUS", "0");
            //        oParamDic.Add("F_USER", gsUSERID);

            //        oSPs[idx] = "USP_QCD74_UPD";
            //        oParameters[idx] = (object)oParamDic;
        ...
        {
            devGridExporter.WriteXlsToResponse(String.Format("[{0}]{1} 공정정보", DateTime.Today.ToString("yyyyMMdd"), gsCOMPNM), new DevExpress.XtraPrinting.XlsExportOptionsEx { ExportType = DevExpress.Export.ExportType.WYSIWYG });
        }
        #endregion

        protected void devGrid_DataBound(object sender, EventArgs e)
        {
            var combo2 = devGrid.Columns["F_NGCODE"] as DevExpress.Web.GridViewDataComboBoxColumn;
            combo2.PropertiesComboBox.TextField = "F_NGDETAIL";
            combo2.PropertiesComboBox.ValueField = "F_NGCODE";
            combo2.PropertiesComboBox.DataSource = QCE33_LST().Tables[0];
        }
