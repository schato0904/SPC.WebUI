...
{
    public partial class BSIF0401_DACO : WebUIBasePage
    {
        ...
        #region 부적합 유형 목록을 구한다 구한다
        void BSIF0401_DACO_LST()
        {
            ...
            using (BSIFBiz biz = new BSIFBiz())
            {
                ...
                ds = biz.BSIF0401_DACO_LST(oParamDic, out errMsg);
            }
        ...
        protected void devGrid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (e.InsertValues.Count > 0)
            {
                foreach (var Value in e.InsertValues)
                {
                    ...
                    oParamDic.Add("F_REMARK", (Value.NewValues["F_QRCODE"] ?? "").ToString());
        ...
        {
            devGridExporter.WriteXlsToResponse(String.Format("[{0}]{1} 부적합유형정보", DateTime.Today.ToString("yyyyMMdd"), gsCOMPNM), new DevExpress.XtraPrinting.XlsExportOptionsEx { ExportType = DevExpress.Export.ExportType.WYSIWYG });
        }
        #endregion
