...
{
    public partial class BSIF0701 : WebUIBasePage
    {
        ...
        void QCDALRAMTIME_LST()
        {
            ...
            using (BSIFBiz biz = new BSIFBiz())
            {
                ...
                ds = biz.QCDALRAMTIME_LST(oParamDic, out errMsg);
            }
        ...
        protected void devGrid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (e.InsertValues.Count > 0)
            {
                foreach (var Value in e.InsertValues)
                {
                    ...
                    oParamDic.Add("F_COMPCD", gsCOMPCD);
                    oParamDic.Add("F_FACTCD", gsFACTCD);
                    oParamDic.Add("F_HOUR", (Value.NewValues["F_HOUR"] ?? "").ToString());
                    oParamDic.Add("F_MINUTE", (Value.NewValues["F_MINUTE"] ?? "").ToString());                    
                    oParamDic.Add("F_USER", gsUSERID);

                    oSPs[idx] = "USP_QCDALRAMTIME_INS";
            ...
            if (e.DeleteValues.Count > 0)
            {
                foreach (var Value in e.DeleteValues)
                {
                    ...
                    oParamDic.Add("F_COMPCD", gsCOMPCD);
                    oParamDic.Add("F_FACTCD", gsFACTCD);
                    oParamDic.Add("F_HOUR", (Value.Values["F_HOUR"] ?? "").ToString());

                    oSPs[idx] = "USP_QCDALRAMTIME_DEL";
                    oParameters[idx] = (object)oParamDic;
            ...
            using (BSIFBiz biz = new BSIFBiz())
            {
                bExecute = biz.PROC_QCDALRAMTIME_BATCH(oSPs, oParameters, out resultMsg);
            }
        ...
        {
            devGridExporter.WriteXlsToResponse(String.Format("[{0}]{1} 경광등알람시간정보", DateTime.Today.ToString("yyyyMMdd"), gsCOMPNM), new DevExpress.XtraPrinting.XlsExportOptionsEx { ExportType = DevExpress.Export.ExportType.WYSIWYG });
        }
        #endregion
