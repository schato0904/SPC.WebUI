...
{
    public partial class BSIF0501_Work : WebUIBasePage
    {
        ...
        {
            DevExpressLib.GetUsedString(new string[] { "F_STATUS","F_USEYN" }, e);
        ...
        protected void devGrid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (e.InsertValues.Count > 0)
            {
                foreach (var Value in e.InsertValues)
                {
                    ...
                    oParamDic.Add("F_COMPCD", gsCOMPCD);
                    oParamDic.Add("F_FACTCD", gsFACTCD);
                    oParamDic.Add("F_PCNO", (Value.NewValues["F_PCNO"] ?? "").ToString());
                    ...
                    oParamDic.Add("F_POSITION", (Value.NewValues["F_POSITION"] ?? "").ToString());
                    oParamDic.Add("F_STATUS", (Value.NewValues["F_STATUS"] ?? "").ToString());
                    oParamDic.Add("F_USER", gsUSERID);
                    oSPs[idx] = "USP_QCDPCNM_WORK_INS";
            ...
            if (e.DeleteValues.Count > 0)
            {
                foreach (var Value in e.DeleteValues)
                {
                    ...
                    oParamDic.Add("F_COMPCD", gsCOMPCD);
                    oParamDic.Add("F_FACTCD", gsFACTCD);
                    oParamDic.Add("F_PCNO", (Value.Values["F_PCNO"] ?? "").ToString());
                    oParamDic.Add("F_WORKCD", (Value.Values["F_WORKCD"] ?? "").ToString());

                    oSPs[idx] = "USP_QCDPCNM_WORK_DEL";
                    oParameters[idx] = (object)oParamDic;
        ...
        {
            var combo = devGrid.Columns["F_WORKCD"] as DevExpress.Web.GridViewDataComboBoxColumn;
            ...
            using (BSIFBiz biz = new BSIFBiz())
            {
                ...
                ds = biz.MORNITORING_WORK_LST(oParamDic, out errMsg);
            }

            combo.PropertiesComboBox.TextField = "F_WORKNM";
            combo.PropertiesComboBox.ValueField = "F_WORKCD";
        ...
        {
            devGridExporter.WriteXlsToResponse(String.Format("[{0}]{1} 모니터링공정정보", DateTime.Today.ToString("yyyyMMdd"), gsCOMPNM), new DevExpress.XtraPrinting.XlsExportOptionsEx { ExportType = DevExpress.Export.ExportType.WYSIWYG });
        }
        #endregion
