...
{
    public partial class BSIF0204A : WebUIBasePage
    {
        ...
        void QCD17A_LST()
        {
            ...
            using (CommonBiz biz = new CommonBiz())
            {
                ...
                ds = biz.QCD17A_LST(oParamDic, out errMsg);
            }
        ...
        protected void devGrid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (e.InsertValues.Count > 0)
            {
                foreach (var Value in e.InsertValues)
                {
                    ...
                    oParamDic.Add("F_MODELNM", (Value.NewValues["F_MODELNM"] ?? "").ToString());
                    oParamDic.Add("F_USER", gsUSERID);

                    oSPs[idx] = "USP_QCD17A_INS";
            ...
            if (e.DeleteValues.Count > 0)
            {
                foreach (var Value in e.DeleteValues)
                {
                    ...
                    oParamDic.Add("F_COMPCD", gsCOMPCD);
                    oParamDic.Add("F_FACTCD", gsFACTCD);
                    oParamDic.Add("F_MODELCD", (Value.Values["F_MODELCD"] ?? "").ToString());
                    oParamDic.Add("F_STATUS", (Value.Values["F_STATUS"] ?? "").ToString());
                    oParamDic.Add("F_MODELNM", (Value.Values["F_MODELNM"] ?? "").ToString());
                    oParamDic.Add("F_USER", gsUSERID);

                    oSPs[idx] = "USP_QCD17A_DEL";
                    oParameters[idx] = (object)oParamDic;
            ...
            {
                bExecute = biz.PROC_QCD17A_BATCH(oSPs, oParameters, out resultMsg);
            }
        ...
        {
            devGridExporter.WriteXlsToResponse(String.Format("[{0}]{1} 기종정보", DateTime.Today.ToString("yyyyMMdd"), gsCOMPNM), new DevExpress.XtraPrinting.XlsExportOptionsEx { ExportType = DevExpress.Export.ExportType.WYSIWYG });
        }
        #endregion
