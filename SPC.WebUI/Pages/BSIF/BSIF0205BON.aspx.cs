...
{
    public partial class BSIF0205BON : WebUIBasePage
    {
        ...
        protected void devGrid_CustomColumnDisplayText(object sender, DevExpress.Web.ASPxGridViewColumnDisplayTextEventArgs e)
        {
            ...
            //DevExpressLib.GetUsedString(new string[] { "F_INSPGUBN" }, e);
            DevExpressLib.GetBoolString(new string[] { "F_INSPGUBN" },"사용","미사용", e);

            /*
            if (e.Column.FieldName == "F_INSPSTND")
            {
                 if (e.Value.ToString() == "1") {
                    e.EncodeHtml = false;
                    e.DisplayText = @"<span style='color:blue;'>사용</span>";
                }
                else
                 {
                     e.EncodeHtml = false;
                     e.DisplayText = @"<span style='color:red;'>중단</span>";
                 }
            }
             * * */
        }
        ...
        protected void devGrid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (e.InsertValues.Count > 0)
            {
                foreach (var Value in e.InsertValues)
                {
                    ...
                    oParamDic.Add("F_INSPGUBN", (Value.NewValues["F_INSPGUBN"] ?? "").ToString());
        ...
        {
            if (!e.Column.FieldName.Equals("F_MODELCD") && !e.Column.FieldName.Equals("F_STATUS") && !e.Column.FieldName.Equals("F_INSPGUBN")) return;
            ...
            else if (e.Column.FieldName.Equals("F_STATUS"))
            {
                ...
                ddlComboBox.Items.Insert(0, new DevExpress.Web.ListEditItem() { Text = "", Value = null, Selected = true });

                foreach (string filter in filters)
                {
                    if (filter.Contains("F_STATUS"))
                    {
                        filterParams = filter.Split('=');
            ...
            else if (e.Column.FieldName.Equals("F_INSPGUBN"))
            {

                DevExpress.Web.ASPxComboBox ddlComboBox = e.Editor as DevExpress.Web.ASPxComboBox;
                ddlComboBox.Items.Clear();
                ddlComboBox.Items.Insert(0, new DevExpress.Web.ListEditItem() { Text = "미사용", Value = false, Selected = false });
                ddlComboBox.Items.Insert(0, new DevExpress.Web.ListEditItem() { Text = "사용", Value = true, Selected = false });
                ddlComboBox.Items.Insert(0, new DevExpress.Web.ListEditItem() { Text = "", Value = null, Selected = true });

                foreach (string filter in filters)
                {
                    if (filter.Contains("F_INSPGUBN"))
                    {
                        filterParams = filter.Split('=');
                        ddlComboBox.SelectedIndex = ddlComboBox.Items.FindByValue(Convert.ToBoolean(filterParams[1].Trim().Replace("'", ""))).Index;
                    }
