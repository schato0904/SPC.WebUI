...
{
    public partial class BSIF0203 : WebUIBasePage
    {
        ...
        DataTable QCD75_LST()
        {
            ...
            dtTable.Columns.Add("F_EQUIPCD", typeof(string));
            dtTable.Columns.Add("F_EQUIPNM", typeof(string));

            foreach (DataRow dtRow in ds.Tables[0].Rows)
            {
                dtTable.Rows.Add(dtRow["F_EQUIPCD"].ToString(), dtRow["F_EQUIPNM"].ToString());
            }

            DataRow dr = dtTable.NewRow();
            dr["F_EQUIPCD"] = String.Empty;
            dr["F_EQUIPNM"] = "선택안함";
        ...
        {
            DevExpress.Web.GridViewDataComboBoxColumn comboBox = devGrid.Columns["F_EQUIPCD"] as DevExpress.Web.GridViewDataComboBoxColumn;
            comboBox.PropertiesComboBox.NullText = "선택하세요";
            comboBox.PropertiesComboBox.TextField = "F_EQUIPNM";
            comboBox.PropertiesComboBox.ValueField = "F_EQUIPCD";
            comboBox.PropertiesComboBox.DataSource = QCD75_LST();
        }
        #endregion
        ...
        protected void devGrid_CustomColumnDisplayText(object sender, DevExpress.Web.ASPxGridViewColumnDisplayTextEventArgs e)
        {
            ...
            if (e.Column.FieldName.Equals("F_EQUIPCD"))
            {
        ...
        protected void devGrid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (e.InsertValues.Count > 0)
            {
                foreach (var Value in e.InsertValues)
                {
                    ...
                    oParamDic.Add("F_EQUIPCD", (Value.NewValues["F_EQUIPCD"] ?? "").ToString());
            ...
            if (e.DeleteValues.Count > 0)
            {
                foreach (var Value in e.DeleteValues)
                {
                    ...
                    oParamDic.Add("F_WORKCD", (Value.Values["F_WORKCD1"] ?? "").ToString() + (Value.Values["F_WORKCD2"] ?? "").ToString());
                    oParamDic.Add("F_OUTMSG", "OUTPUT");

                    oSPs[idx] = "USP_QCD74_DEL";
                    oParameters[idx] = (object)oParamDic;
                ...
                foreach (Dictionary<string, object> _oOutParamDic in oOutParamList)
                {
                    foreach (KeyValuePair<string, object> _oOutPair in _oOutParamDic)
                    {
                        if (!String.IsNullOrEmpty(_oOutPair.Value.ToString()))
                            sb_OutMsg.AppendFormat("공정코드 {0}는 검사기준에서 사용중이므로 삭제할 수 없습니다.\r", _oOutPair.Value);
                    }
                }
