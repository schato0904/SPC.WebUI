...
{
    public partial class BSIF0102 : WebUIBasePage
    {
        ...
        #region 공정이상 기본설정 목록을 구한다
        void QCDSHEWHART_LST()
        {
            ...
            using (BSIFBiz biz = new BSIFBiz())
            {
                ...
                ds = biz.QCDSHEWHART_LST(oParamDic, out errMsg);
            }
        ...
        {
            DevExpressLib.GetUsedString(new string[] { "F_STATUS", "F_DISPLAY" }, e);
        ...
        protected void devGrid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            //        oParamDic.Add("F_COMPCD", gsCOMPCD);
            //        oParamDic.Add("F_FACTCD", gsFACTCD);
            //        oParamDic.Add("F_NOWORKCD", (Value.NewValues["F_NOWORKCD"] ?? "").ToString());
            //        oParamDic.Add("F_STATUS", (Value.NewValues["F_STATUS"] ?? "").ToString());
            //        oParamDic.Add("F_NOWORKNM", (Value.NewValues["F_NOWORKNM"] ?? "").ToString());
            //        oParamDic.Add("F_USER", gsUSERID);

            //        oSPs[idx] = "USP_QCDSHEWHART_INS";
            ...
            if (e.UpdateValues.Count > 0)
            {
                foreach (var Value in e.UpdateValues)
                {
                    ...
                    oParamDic.Add("F_COMPCD", gsCOMPCD);
                    oParamDic.Add("F_FACTCD", gsFACTCD);
                    oParamDic.Add("F_SHEWHARTCD", (Value.NewValues["F_SHEWHARTCD"] ?? "").ToString());
                    oParamDic.Add("F_NO", (Value.NewValues["F_NO"] ?? "").ToString());
                    oParamDic.Add("F_DISPLAY", (Value.NewValues["F_DISPLAY"] ?? "").ToString());
                    oParamDic.Add("F_STATUS", (Value.NewValues["F_STATUS"] ?? "").ToString());
                    oParamDic.Add("F_SHEWHARTNM", (Value.NewValues["F_SHEWHARTNM"] ?? "").ToString());
                    oParamDic.Add("F_REMARK", (Value.NewValues["F_REMARK"] ?? "").ToString());
                    oParamDic.Add("F_USER", gsUSERID);

                    oSPs[idx] = "USP_QCDSHEWHART_UPD";
                    oParameters[idx] = (object)oParamDic;
            ...
            //        oParamDic.Add("F_COMPCD", gsCOMPCD);
            //        oParamDic.Add("F_FACTCD", gsFACTCD);
            //        oParamDic.Add("F_NOWORKCD", (Value.Values["F_NOWORKCD"] ?? "").ToString());
            //        oParamDic.Add("F_STATUS", (Value.Values["F_STATUS"] ?? "").ToString());

            //        oSPs[idx] = "USP_QCDSHEWHART_DEL";
            //        oParameters[idx] = (object)oParamDic;
            ...
            using (BSIFBiz biz = new BSIFBiz())
            {
                bExecute = biz.PROC_QCDSHEWHART_BATCH(oSPs, oParameters, out resultMsg);
            }
        ...
        protected void devGridExporter_RenderBrick(object sender, DevExpress.Web.ASPxGridViewExportRenderingEventArgs e)
        {
            ...
            if (devGridDataColumn != null && devGridDataColumn.FieldName.Equals("F_STATUS") && e.RowType == DevExpress.Web.GridViewRowType.Data ||
                devGridDataColumn != null && devGridDataColumn.FieldName.Equals("F_DISPLAY") && e.RowType == DevExpress.Web.GridViewRowType.Data
                )
            {
        ...
        {
            devGridExporter.WriteXlsToResponse(String.Format("[{0}]{1} 공정이상기본설정", DateTime.Today.ToString("yyyyMMdd"), gsCOMPNM), new DevExpress.XtraPrinting.XlsExportOptionsEx { ExportType = DevExpress.Export.ExportType.WYSIWYG });
        }
        #endregion
