...
{
    public partial class INSP0302POP : WebUIBasePage
    {
        ...
        void GetRequest()
        {
            ...
            srcF_ITEMCD.Text = GetDicValue(PopParam, "F_ITEMCD");
            hidWORKCD.Text = GetDicValue(PopParam, "F_WORKCD");
            srcF_WORKCD.Text = GetDicValue(PopParam, "F_WORKCD");
            hidWORKDATE.Text = GetDicValue(PopParam, "F_WORKDATE");
            srcF_WORKDATE.Text = GetDicValue(PopParam, "F_WORKDATE");
            hidTSERIALNO.Text = GetDicValue(PopParam, "F_TSERIALNO");
            srcF_LOTNO.Text = GetDicValue(PopParam, "F_LOTNO");
            srcF_MOLDNO.Text = GetDicValue(PopParam, "F_MOLDNO");
            srcF_WORKMAN.Text = GetDicValue(PopParam, "F_WORKMAN");
        }
        #endregion
        ...
        #region 상세목록
        void QWK03A_INSP0302POP_DETAIL_LST()
        {
            ...
            using (INSPBiz biz = new INSPBiz())
            {
                ...
                oParamDic.Add("F_WORKDATE", hidWORKDATE.Text);
                oParamDic.Add("F_TSERIALNO", hidTSERIALNO.Text);

                ds = biz.QWK03A_INSP0302POP_DETAIL_LST(oParamDic, out errMsg);
            }
            ...
            foreach (object oJsonDataList in oList)
            {
                ...
                else
                {
                    if (i.Equals(0))
                    {
                        ...
                        dtTemp.DefaultView.Sort = "F_TSERIALNO ASC, F_DISPLAYNO ASC, F_NUMBER";
                    }
                    else
                    {
        ...
        bool QWK13IM_NEW(Dictionary<string, object> paramDic, out string errMsg, out string pkey)
        {
            bool bExecute = false;
            ...
            using (System.Transactions.TransactionScope scope = new System.Transactions.TransactionScope())
            {
                do
                {
                    ...
                    // 디테일 저장 호출
                    if (!QWK13ID_INS_UPD(pkey, paramDic, out errMsg)) break;
                    // 중간검사(EF)샘플검사 저장호출
        ...
        bool QWK13ID_INS_UPD(string F_QWK13IMID, Dictionary<string, object> paramDic, out string errMsg)
        {
            bool bExecute = false;
            ...
            using (INSPBiz biz = new INSPBiz())
            {
                ...
                oParamDic["F_ITEMCD"] = GetDicValue(paramDic, "F_ITEMCD");
                oParamDic["F_WORKCD"] = GetDicValue(paramDic, "F_WORKCD");
                oParamDic["F_QWK13IDID"] = "0";
                oParamDic["F_TSERIALNO"] = GetDicValue(paramDic, "F_TSERIALNO");
                oParamDic["F_USER"] = gsUSERID;

                bExecute = biz.QWK13ID_INS_UPD(oParamDic, out errMsg);
            }
        ...
        protected void devGrid2_CustomCallback(object sender, DevExpress.Web.ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            if (!String.IsNullOrEmpty(e.Parameters))
            {
                ...
                oList = jss.Deserialize<List<object>>(e.Parameters); // 웹에서 전달된 파라미터
            }
            else
            {
        ...
        protected void devCallback_Callback(object source, DevExpress.Web.CallbackEventArgs e)
        {
            ...
            {
                case "NEW":
                    if (!QWK13IM_NEW(paramDic, out errMsg, out pkey))
                    {
                        ISOK = false;
                        result["ISOK"] = ISOK;
                        result["MSG"] = errMsg;
