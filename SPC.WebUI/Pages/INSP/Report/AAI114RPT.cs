...
{
    public partial class AAI114RPT : DevExpress.XtraReports.UI.XtraReport
    {
        #region 변수
        ...
        void SetHeader()
        {
            ...
            // 사출업체
            DevExpressLib.SetText(xrTableCell23, drQWK13IM["F_INJCUST"].ToString());
            // 사출일자
            DevExpressLib.SetText(xrTableCell35, drQWK13IM["F_INJDATE"].ToString());
            // 입고일자
            DevExpressLib.SetText(xrTableCell47, drQWK13IM["F_INDATE"].ToString());
            // 제품명
            ...
            // 재질
            DevExpressLib.SetText(xrTableCell58, drQWK13IM["F_INMATERIAL"].ToString());
            // 원료
            DevExpressLib.SetText(xrTableCell60, drQWK13IM["F_MATERIAL"].ToString());
            // 사출온도
            DevExpressLib.SetText(xrTableCell75, drQWK13IM["F_INTEMP"].ToString());
            // 사이클시간
            DevExpressLib.SetText(xrTableCell76, String.Format("{0}{1}", drQWK13IM["F_CYCEL"], drQWK13IM["F_CYCELUNIT"]));
            // 사출설비명
            DevExpressLib.SetText(xrTableCell77, drQWK13IM["F_MACHINE"].ToString());
            // 업체배포
            DevExpressLib.SetText(xrTableCell78, drQWK13IM["F_DISTRIBUTE"].ToString());
            // 입고수량
            DevExpressLib.SetText(xrTableCell81, String.Format("{0:#,###} {1}", !String.IsNullOrEmpty(drQWK13IM["F_INCNT"].ToString()) ? Convert.ToInt32(drQWK13IM["F_INCNT"]) : 0, drQWK13IM["F_INUNIT"]));
            // 검사원
        ...
        void SetContent()
        {
            ...
            {
                if (dr["F_ASSORTMENT"].ToString().Equals("AAI204"))    // 외관인 경우
                {
                ...
                else if (dr["F_ASSORTMENT"].ToString().Equals("AAI202") && this.dtQWK13ID.Rows.Count > 0)    // 치수인 경우
                {
                    ...
                    xrCell = new XRTableCell();
                    xrCell.WidthF = 280F;
                    xrCell.Text = "체크 항목 외 치수는 사출 도면을 참고하여 사출할 것.";
                    xrCell.CanGrow = false;
                    xrCell.RowSpan = 2;
                    xrRow.Cells.Add(xrCell);
                ...
                else if (dr["F_ASSORTMENT"].ToString().Equals("AAI201"))    // 일반인 경우
                {
                    xrRow = new XRTableRow();
                    xrRow.HeightF = 35F;
