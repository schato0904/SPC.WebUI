...
{
    public partial class ADTR0203_FND : WebUIBasePage
    {
        ...
        protected void Page_Init(object sender, EventArgs e)
        {
            ...
            {
                QWK03A_ADTR0203_LST(ucPager.GetPageSize(), 0, false);
            }
        }
        #endregion
        ...
        Int32 QWK03A_ADTR0203_CNT()
        {
            using (ADTRBiz biz = new ADTRBiz())
            {
                ...
                oParamDic.Add("F_GBN", (this.ddlJUDGE.Value ?? "").ToString());
                oParamDic.Add("F_LOTNO", (this.txtLOTNO.Text ?? "").ToString());
                totalCnt = biz.QWK03A_ADTR0203_CNT(oParamDic);
            }
        ...
        #region Data조회
        void QWK03A_ADTR0203_LST(int nPageSize, int nCurrPage, bool bCallback)
        {
            ...
            using (ADTRBiz biz = new ADTRBiz())
            {
                ...
                ds = biz.QWK03A_ADTR0203_LST(oParamDic, out errMsg);
            }
            ...
            else
            {
                ...
                else
                {
                    ...
                    devGrid.JSProperties["cpResultMsg"] = String.Format("PAGESIZE={0};CURRPAGE={1};ITEMSCNT={2}", nPageSize, nCurrPage, QWK03A_ADTR0203_CNT());
                }
            }
        }
        #endregion
        ...
        void QWK03A_ADTR0203_LST()
        {
            ...
            using (ADTRBiz biz = new ADTRBiz())
            {
                ...
                oParamDic.Add("F_GBN", (this.ddlJUDGE.Value ?? "").ToString());
                oParamDic.Add("F_LOTNO", (this.txtLOTNO.Text ?? "").ToString());
                oParamDic.Add("F_PAGESIZE", QWK03A_ADTR0203_CNT().ToString());
                oParamDic.Add("F_CURRPAGE", "1");
                ds2 = biz.QWK03A_ADTR0203_LST(oParamDic, out errMsg);
            }
            devGrid3.DataSource = ds2;
        }
        #endregion

        #region 상세내용
        void QWK03A_ADTR0203_DETAIL_LST(string[] Param)
        {
            string errMsg = String.Empty;
            ...
            using (ADTRBiz biz = new ADTRBiz())
            {
                ...
                oParamDic.Add("F_SERIALNO", Param[4]);
                oParamDic.Add("F_TSERIALNO", Param[5]);
                oParamDic.Add("F_FROMDT", Convert.ToDateTime(Param[6]).AddMonths(-2).ToString("yyyy-MM-dd"));
                oParamDic.Add("F_TODT", Param[6]);
                
                oParamDic.Add("F_UCLLCL", gsUCLLCL);
                
                ds = biz.QWK03A_ADTR0203_DETAIL_LST(oParamDic, out errMsg);
            }
            ...
            {
                DevExpress.Web.GridViewDataColumn column = new DevExpress.Web.GridViewDataColumn() { FieldName = "F_MEASURE" +i.ToString(), Caption = "DATA" + i.ToString(), Width = Unit.Parse("80") };
                devGrid2.Columns.Add(column);
            }

            devGrid2.DataSource = dt;
        ...
        DataTable PivotDataSet(DataSet ds)
        {
            ...
            int idx = 0;
            string[] ColumnNm = new string[ds.Tables[0].Columns.Count];
            foreach (DataColumn dc in ds.Tables[0].Columns)
            {
                dt.Columns.Add(dc.ColumnName.ToString());
                ColumnNm[idx] = dc.ColumnName.ToString();
                idx++;
            }
            ...
            foreach (DataRow dr in ds.Tables[0].Rows)
	        {
                if (idx == 1)
                {
                    ...
                    {
                        dt.Rows[0][ColumnNm[i]] = dr[ColumnNm[i]].ToString();
                    }
                }
                else
                {
                    dt.Columns.Add("F_MEASURE"+idx.ToString());
                    dt.Rows[0]["F_MEASURE" + idx.ToString()] = dr["F_MEASURE1"].ToString();
                }
                idx++;
	        }
            return dt;
        ...
        {
            // Tooltip 출력
            string[] sTooltipFields = { "F_ITEMNM", "F_WORKNM", "F_INSPDETAIL", "F_LOTNO","F_REMARK" };
        ...
        protected void devGrid_CustomColumnDisplayText(object sender, DevExpress.Web.ASPxGridViewColumnDisplayTextEventArgs e)
        {
            ...
            if (e.Column.FieldName.Equals("F_JUDGE"))
            {
                e.DisplayText = GetCommonCodeText(CachecommonCode["AA"]["AAE2"][e.Value.ToString()]);
            }
        }
        #endregion
        ...
        protected void devGrid_CustomCallback(object sender, DevExpress.Web.ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            QWK03A_ADTR0203_LST(nPageSize, nCurrPage, true);
            devGrid.DataBind();
        }
        #endregion        
        ...
        protected void devGrid2_CustomCallback(object sender, DevExpress.Web.ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            QWK03A_ADTR0203_DETAIL_LST(key);
        }
        #endregion     
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            QWK03A_ADTR0203_LST();
            devGrid3.DataBind();
            devGridExporter.WriteXlsToResponse(String.Format("[{0}]{1} 공정이상조치보고서정보", DateTime.Today.ToString("yyyyMMdd"), gsCOMPNM), new DevExpress.XtraPrinting.XlsExportOptionsEx { ExportType = DevExpress.Export.ExportType.WYSIWYG });
        }
        #endregion
