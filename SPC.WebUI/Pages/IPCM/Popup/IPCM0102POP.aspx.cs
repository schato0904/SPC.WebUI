...
{
    public partial class IPCM0102POP : WebUIBasePage
    {
        ...
        void QWK100_GET()
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    // 협력업체이며 관리번호가 없는 경우 반드시 입력하게 한다.
                    if (!Convert.ToBoolean(dtRow["F_BVENDOR"]) && String.IsNullOrEmpty(dtRow["F_CTRLNO"].ToString()))
                    {
                        txtCTRLNO.ReadOnly = false;
                        bCTRLNO.Text = "1";
                    }

                    txtIMAGESEQ.Text = dtRow["F_RSFILE"].ToString();
        ...
        protected void devCallback_Callback(object source, DevExpress.Web.CallbackEventArgs e)
        {
            ...
            if (oParams[0].Equals("SAVE"))
            {
                ...
                oParamDic.Add("F_RSDATE", DateTime.Today.ToString("yyyy-MM-dd"));
                oParamDic.Add("F_RSCPCD", gsCOMPCD);
                oParamDic.Add("F_RSFTCD", gsFACTCD);
                oParamDic.Add("F_RSUSID", gsUSERID);
                oParamDic.Add("F_RSUSNM", gsUSERNM);
                oParamDic.Add("F_RSTXT1", txtRSTXT1.Text);
                oParamDic.Add("F_RSTXT2", txtRSTXT2.Text);
                oParamDic.Add("F_RSTXT3", txtRSTXT3.Text);
                oParamDic.Add("F_RSTXT4", txtRSTXT4.Text);
                oParamDic.Add("F_RSFILE", txtIMAGESEQ.Text);

                using (IPCMBiz biz = new IPCMBiz())
                {
                    bExecute = biz.QWK100_STEP2_UPD(oParamDic, out errMsg);
                }
