...
{
    public partial class FDCK0101 : WebUIBasePage
    {
        ...
        {
            this.srcF_COMPCD.Text = gsCOMPCD;
            this.srcF_COMPNM.Text = gsCOMPNM;
            this.srcF_FACTCD.SetValue(gsFACTCD);
        }
        #endregion
        ...
        void RetrieveList()
        {
            ...
            using (FDCKBiz biz = new FDCKBiz())
            {
                ...
                oParamDic.Add("F_FACTCD", schF_FACTCD.GetValue());
        ...
        // 선택한 설비 상세정보 조회
        DataSet RetrieveDetail(string f_compcd, string f_factcd, string f_machcd, out string errMsg)
        {
            errMsg = String.Empty;
            ds.Clear();
            using (FDCKBiz biz = new FDCKBiz())
        ...
        bool QCD_MACH01_INS(out string errMsg)
        {
            ...
            {
                oParamDic.Clear();
                oParamDic.Add("F_COMPCD", srcF_COMPCD.Text);
                oParamDic.Add("F_FACTCD", srcF_FACTCD.GetValue());
                ...
                oParamDic.Add("F_INDATE", srcF_INDATE.Text);
                ...
                oParamDic.Add("F_IMAGENO", txtIMAGESEQ.Text);
                oParamDic.Add("F_REMARK", srcF_REMARK.Text);
                oParamDic.Add("F_INSUSER", gsUSERID);
                oParamDic.Add("F_WORKERS", srcWORKERS_USER.Text);
                oParamDic.Add("F_ADMINS", srcADMINS_USER.Text);

                bExecute = biz.QCD_MACH01_INS(oParamDic, out errMsg);
            }
        ...
        bool QCD_MACH01_UPD(out string errMsg)
        {
            ...
            using (FDCKBiz biz = new FDCKBiz())
            {
                ...
                oParamDic.Add("F_WORKERS", srcWORKERS_USER.Text);
                oParamDic.Add("F_ADMINS", srcADMINS_USER.Text);
                bExecute = biz.QCD_MACH01_UPD(oParamDic, out errMsg);
            }
        ...
        bool QCD_MACH01_DEL(out string errMsg)
        {
            ...
            using (FDCKBiz biz = new FDCKBiz())
            {
                ...
                oParamDic.Add("F_FACTCD", srcF_FACTCD.GetValue());
                oParamDic.Add("F_MACHCD", srcF_MACHCD.Text);
                bExecute = biz.QCD_MACH01_DEL(oParamDic, out errMsg);
            }
        ...
        {
            //QWK04A_ADTR0103_LST();
            devGridExporter.WriteXlsToResponse(String.Format("[{0}]{1} 설비등록", DateTime.Today.ToString("yyyyMMdd"), gsCOMPNM), new DevExpress.XtraPrinting.XlsExportOptionsEx { ExportType = DevExpress.Export.ExportType.WYSIWYG });
        }
        #endregion
            ...
            switch (paramDic["ACTION"])
            {
                case "SAVE":
                    if (paramDic["PAGEMODE"] == "NEW")
                    {
                        ...
                        {
                            ISOK = true;
                            PKEY["F_COMPCD"] = (this.srcF_COMPCD.Value ?? "").ToString();
                            PKEY["F_FACTCD"] = (this.srcF_FACTCD.GetValue() ?? "").ToString();
                            PKEY["F_MACHCD"] = (this.srcF_MACHCD.Value ?? "").ToString();
                            result["ISOK"] = ISOK;
                            result["PKEY"] = PKEY;
                            result["MSG"] = "저장되었습니다.";
                        }
                ...
                case "GET":
                    ...
                    ds.Clear();
                    ds = RetrieveDetail(paramDic["F_COMPCD"], paramDic["F_FACTCD"], paramDic["F_MACHCD"], out errMsg);
                    if (!string.IsNullOrEmpty(errMsg))
                    {
                        ISOK = false;
                        msg = errMsg;
