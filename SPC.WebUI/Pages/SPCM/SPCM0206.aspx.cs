...
{
    public partial class SPCM0206 : WebUIBasePage
    {
        ...
        void SPCM0206D_LST()
        {
            ...
            using (SPCMBiz biz = new SPCMBiz())
            {
                ...
                oParamDic.Add("F_OTYPE", (this.ddlType.Value ?? "").ToString());
                oParamDic.Add("F_FRDT", GetFromDt());
                oParamDic.Add("F_TODT", GetToDt());
                oParamDic.Add("F_IOGBN", this.chk_gbn.Value == null ? "A" : this.chk_gbn.SelectedItem.Value.ToString());
                oParamDic.Add("F_ITCD", txtITEMCD.Text);
                ds = biz.SPCM0206_LST(oParamDic, out errMsg);
            }
        ...
        void ddlTypeBind()
        {
            ...
            using (SPCMBiz biz = new SPCMBiz())
            {
                ...
                oParamDic.Add("F_COMPCD", gsCSRCOMPCD);
                oParamDic.Add("F_IOGBN", this.chk_gbn.Value == null ? "A" : this.chk_gbn.SelectedItem.Value.ToString());
                tyds = biz.SPB06A_LST(oParamDic, out errMsg);
            }
            ddlType.DataSource = tyds;
            ddlType.TextField = "TYPENM";
            ddlType.ValueField = "TYPECD";
            ddlType.DataBind();
            ddlType.Items.Insert(0, new DevExpress.Web.ListEditItem() { Text = "전체", Value = "", Selected = true });
        ...
        {
            devGridExporter.WriteXlsToResponse(String.Format("[{0}]{1} 입/출고이력정보", DateTime.Today.ToString("yyyyMMdd"), gsCOMPNM), new DevExpress.XtraPrinting.XlsExportOptionsEx { ExportType = DevExpress.Export.ExportType.WYSIWYG });
        }
        #endregion
