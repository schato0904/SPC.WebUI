...
{
    public partial class SPCM0103 : WebUIBasePage
    {
        ...
        void SPCM0103_LST()
        {
            ...
            using (SPCMBiz biz = new SPCMBiz())
            {
                ...
                ds = biz.SPB03_LST(oParamDic, out errMsg);
            }
        ...
        protected void devGrid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (e.InsertValues.Count > 0)
            {
                foreach (var Value in e.InsertValues)
                {
                    ...
                    oParamDic.Add("F_COMPCD", gsCSRCOMPCD);
                    oParamDic.Add("F_OTYPECD", (Value.NewValues["F_OTYPECD"] ?? "").ToString());
                    oParamDic.Add("F_OTYPENM", (Value.NewValues["F_OTYPENM"] ?? "").ToString());
                    oParamDic.Add("F_STATUS", (Value.NewValues["F_STATUS"] ?? "").ToString());
                    oSPs[idx] = "USP_SPB03_INS";
            ...
            if (e.DeleteValues.Count > 0)
            {
                foreach (var Value in e.DeleteValues)
                {
                    oParamDic = new Dictionary<string, string>();
                    oParamDic.Add("F_COMPCD", gsCSRCOMPCD);
                    oParamDic.Add("F_OTYPECD", (Value.Values["F_OTYPECD"] ?? "").ToString());

                    oSPs[idx] = "USP_SPB03_DEL";
                    oParameters[idx] = (object)oParamDic;
        ...
        {
            devGridExporter.WriteXlsToResponse(String.Format("[{0}]{1} 출고유형정보", DateTime.Today.ToString("yyyyMMdd"), gsCOMPNM), new DevExpress.XtraPrinting.XlsExportOptionsEx { ExportType = DevExpress.Export.ExportType.WYSIWYG });
        }
        #endregion
