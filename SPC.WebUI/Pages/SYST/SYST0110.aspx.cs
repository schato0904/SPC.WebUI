...
{
    public partial class SYST0110 : WebUIBasePage
    {
        ...
        void QCDMONITORINGNOTICE_LST()
        {
            ...
            using (SYSTBiz biz = new SYSTBiz())
            {
                ...
                oParamDic.Add("F_ENDDATE", ucDate.GetFromDt());
                ...
                ds = biz.QCDMONITORINGNOTICE_LST(oParamDic, out errMsg);
            }
        ...
        {
            e.NewValues["F_ENDDATE"] = Convert.ToDateTime("9999-12-31");
        ...
        protected void devGrid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (e.InsertValues.Count > 0)
            {
                foreach (var Value in e.InsertValues)
                {
                    ...
                    oParamDic.Add("F_ENDDATE", Convert.ToDateTime(Value.NewValues["F_ENDDATE"] ?? "").ToString("yyyy-MM-dd"));
                    oParamDic.Add("F_STATUS", (Value.NewValues["F_STATUS"] ?? "").ToString());
                    oParamDic.Add("F_USER", gsUSERID);

                    oSPs[idx] = "USP_QCDMONITORINGNOTICE_INS";
            ...
            if (e.UpdateValues.Count > 0)
            {
                foreach (var Value in e.UpdateValues)
                {
                    ...
                    oParamDic.Add("F_COMPCD", gsCOMPCD);
                    oParamDic.Add("F_FACTCD", gsFACTCD);
                    oParamDic.Add("F_IDX", Value.Keys["F_IDX"].ToString());
                    oParamDic.Add("F_CONTENTS", (Value.NewValues["F_CONTENTS"] ?? "").ToString());
                    oParamDic.Add("F_ENDDATE", (Value.NewValues["F_ENDDATE"] ?? "").ToString());
                    oParamDic.Add("F_ORDERNO", (Value.NewValues["F_ORDERNO"] ?? "").ToString());
                    oParamDic.Add("F_STATUS", (Value.NewValues["F_STATUS"] ?? "").ToString());
                    oParamDic.Add("F_USER", gsUSERID);

                    oSPs[idx] = "USP_QCDMONITORINGNOTICE_UPD";
                    oParameters[idx] = (object)oParamDic;
            ...
            using (SYSTBiz biz = new SYSTBiz())
            {
                bExecute = biz.PROC_QCDMONITORINGNOTICE_BATCH(oSPs, oParameters, out resultMsg);
            }
