...
namespace SPC.WebUI.Resources.report.form.foseco
{
    public partial class xtraReport2 : DevExpress.XtraReports.UI.XtraReport
    {
        ...
        private void SetReportHeader()
        {
            ...
            // 수량
            DevExpressLib.SetText(xrTableCell11, Convert.ToInt32(dtHeadRow["F_QTY"]).ToString("n0"));
        ...
        private void RendorReportData()
        {
            ...
            for (int i = 0; i < dtGroup.Rows.Count; i++)
            {
                ...
                {
                    if (dtGroup.Rows[i]["F_INSPDETAIL"].ToString() == dtGroup.Rows[i + 1]["F_INSPDETAIL"].ToString())
                    {
                        rowscnt++;
                    }
                    else
            ...
            {
                XRTableCell cell = xrTable.Rows[i + 1].Cells["F_INSPDETAIL"] as XRTableCell;
                cell.RowSpan = rows[rowspanNo];

                XRTableCell cell2 = xrTable.Rows[i + 1].Cells["F_UNIT"] as XRTableCell;
                cell2.RowSpan = rows[rowspanNo];

                XRTableCell cell3 = xrTable.Rows[i + 1].Cells["F_STANDARD"] as XRTableCell;
                cell3.RowSpan = rows[rowspanNo];
