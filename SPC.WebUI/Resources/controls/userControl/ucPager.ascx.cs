...
{
    public partial class ucPager : System.Web.UI.UserControl, IDisposable
    {
        #region 사용자 정의 속성, 프로퍼티

        #region 변수선언
        private Int32 _PageSize = 30;
        ...
        {
            int size = Convert.ToInt16(CommonHelper.GetAppSectionsString("PageSize"));
            if (size == null || size == 0)
            {
                hidPageSize.Text = PageSize.ToString();
            }
            else
            {
                hidPageSize.Text = size.ToString();
            }
        ...
        {
            pagerPanel.JSProperties["cpIsParent"] = "";
        }
        #endregion
        ...
        {
            //return Convert.ToInt32(hidPageSize.Text);
            return Convert.ToInt32((ddlPageSize.Value??"50").ToString());
            
        }
        #endregion

        #region 사용자가 선택한 CurrPage를 구한다
        public Int32 GetCurrPage()
        {
            return Convert.ToInt32(hidCurrPage.Text);
        }
        #endregion
        ...
        void PagerRendering()
        {
            ...
            {
                sb.AppendFormat("<li><a href=\"javascript:void(0);\" onclick=\"fn_gotoPage('{0}');\"><i class=\"fa fa-angle-double-left\"></i></a></li>", "1");
                sb.AppendFormat("<li><a href=\"javascript:void(0);\" onclick=\"fn_gotoPage('{0}');\"><i class=\"fa fa-angle-left\"></i></a></li>", nStartPage - BlockSize);
            }
            ...
            for (int idx = nStartPage; idx <= nEndPage; idx++)
            {
                if (this.CurrPage.Equals(idx))
                    ...
                    sb.AppendFormat("<li><a href=\"javascript:void(0);\" onclick=\"fn_gotoPage('{0}');\">{0}</a></li>", idx);
            }
            ...
            {
                sb.AppendFormat("<li><a href=\"javascript:void(0);\" onclick=\"fn_gotoPage('{0}');\"><i class=\"fa fa-angle-right\"></i></a></li>", nEndPage + 1);
                sb.AppendFormat("<li><a href=\"javascript:void(0);\" onclick=\"fn_gotoPage('{0}');\"><i class=\"fa fa-angle-double-right\"></i></a></li>", TotalPages);
            }
            sb.Append("</ul>");
        ...
        public void PagerDataBind()
        {
            ...
            {
                ddlPageSize.SelectedIndex = ddlPageSize.Items.FindByValue(size.ToString()).Index;
            }

            if (!string.IsNullOrEmpty(CommonHelper.GetAppSectionsString("PageSize")))
            {
                ddlPageSize.SelectedIndex = ddlPageSize.Items.FindByValue(CommonHelper.GetAppSectionsString("PageSize").ToString()).Index;
            }
            ...
            // 페이지정보
            lblPageInfo.Text = String.Format("Page {0} of {1} ({2} items)", CurrPage, TotalPages, TotalItems.ToString("#,##0"));
