<%@ Page Title="" Language="C#" MasterPageFile="~/Resources/controls/MasterPage/spcMasterPage.Master" AutoEventWireup="true" CodeBehind="WSTA0106_Polytec.aspx.cs" Inherits="SPC.WebUI.Pages.WSTA.WSTA0106_Polytec" %>
<asp:Content ID="Content1" ContentPlaceHolderID="cphHead" runat="server">
    ...
    <script type="text/javascript">
        ...
        function fn_AdjustSize() {
            ...
            devGrid.SetWidth(parseInt($(".search").width() - 1, 10)*0.6);
            devGrid.SetHeight(210);

            var Width = parseInt($(".search").width() - 1, 10)
            chartWidth = Math.round(Width * 0.6,2);
            chartWidth2 = Math.round(Width * 0.4,2);
            chartHeight1 = parseInt(height, 10) - (280);
            //alert(chartHeight1)
        ...
        function fn_grid(s, e) {
            ...
            $(s.GetMainElement()).find('table[id$=DXMainTable]>tbody>tr[id$=DXDataRow1]>td:gt(0)').each(function (idx) {
                arrData[idx] = { "NAME": $(this).text(), "CODE": arrData[idx]["CODE"] };
            });

            //$(s.GetMainElement()).find('table[id$=DXMainTable]>tbody>tr[id$=DXDataRow4]>td:gt(0)').each(function (idx) {
            //    arrData[idx] = { "Inspdetail": $(this).text(), "Serialno": arrData[idx]["Serialno"], "Itemcd": arrData[idx]["Itemcd"] };
            //});

            $(s.GetMainElement()).find('table[id$=DXMainTable]>tbody>tr:gt(0)').each(function (idx) {
                $(this).find('td:gt(0)').each(function (idx) {
                    $(this).on("click", function () {
                        fn_Chart2Callback(arrData[idx]["CODE"], arrData[idx]["NAME"]);
                    });
                });
            });
        }

        function fn_Chart2Callback(CODE, NAME) {
            _Itemcd = CODE + "|" + NAME + "|" 
            //alert(_Itemcd)
            //fn_OnChartDoCallback2(devChart2, chartWidth2, chartHeight1)
        }
        ...
        function fn_OnChartDoCallback2(chartObj, _width, _height) {
            var oParams = _width + '|' + (_height+290) + '|' + _Itemcd;
            chartObj.PerformCallback(oParams);
        }
        ...
        <div class="content">
             <table class="tblContents" style="width: 100%;">
                ...
                <tr>
                 ...
                 <tr>
                     <td style="height: 215px; padding-top: 10px;" >
