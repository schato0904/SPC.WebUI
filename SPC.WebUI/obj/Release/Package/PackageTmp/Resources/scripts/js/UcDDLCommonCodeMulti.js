(function (CTF, window, undefined) {
    ...
    CTF.UcCOMMCD = function (clientInstanceName, strHidCOMMCDCD, strTargetCtrls, strDdlCOMMCD, strJsonParameter, objTargetFuncs) {

        this.ClientInstanceName = clientInstanceName;

        var objHidText = ASPxClientTextBox.Cast(strHidCOMMCDCD);
        var objDdl = ASPxClientComboBox.Cast(strDdlCOMMCD);
        
        // 콤보박스 조회 파라미터
        var jsonParameter = strJsonParameter != '' ? JSON.parse(strJsonParameter) : {};

        if (typeof objTargetFuncs != 'function')
            objTargetFuncs = function () { };
        ...
        objDdl.ValueChanged.AddHandler(objTargetFuncs);
            ...
            if ('' != strTargetCtrls) {
                ...
                if (tCtrls.length > 0) {
                    $.each(tCtrls, function (key, tCtrl) {
                        ...
                        } else if (typeof window[tCtrl] != 'undefined' && typeof window[tCtrl] == 'function') {
                            window[tCtrl]();
                        } else {
        ...
        this.SetValue = function (value) {
            fn_SetTextValue(strHidCOMMCDCD, value);
            fn_SetValue(strDdlCOMMCD, value);
