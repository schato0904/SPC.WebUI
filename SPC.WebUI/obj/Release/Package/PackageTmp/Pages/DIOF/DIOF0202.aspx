<%@ Page Title="" Language="C#" MasterPageFile="~/Resources/controls/MasterPage/spcMasterPage.Master" AutoEventWireup="true" CodeBehind="DIOF0202.aspx.cs" Inherits="SPC.WebUI.Pages.DIOF.DIOF0202" %>
    ...
    <script type="text/javascript">
        ...
        function fn_CreateBullet() {
            ...
            for (var i = 0; i < rowCount; i++) {
                ...
                newdata.POSX = rowKeys[3];
                newdata.POSY = rowKeys[4];
                newdata.MACHCD = rowKeys[1];
                newdata.MACHNM = rowKeys[2];
                newdata.NO = rowKeys[5];
            ...
            $(posdata).each(function () {
                ...
                var param = { "XVal": pos_x, "YVal": pos_y, "MACHIDX": this.MACHIDX, "MACHCD": this.MACHCD, "MACHNM": this.MACHNM, "NO": this.NO, "ID": "div_" + this.MACHIDX };
                ...
                newdiv.attr('title', this.MACHNM + '[' + this.MACHCD + ']');
                $(newdiv).draggable({
                    containment: '#divLayout',
                    snap: '#divLayout'
                }).on("dragstart", function (event, ui) {
                    $(this).css('background-image', 'url(' + icon_red + ')');
                    dragStart(param, parseInt(ui.offset.top, 10) - parseInt(offset_x, 10), parseInt(ui.offset.left, 10) - parseInt(offset_y, 10));
                }).on("dragstop", function (event, ui) {
                    dragStop(param, parseInt(ui.offset.top, 10) - parseInt(offset_x, 10), parseInt(ui.offset.left, 10) - parseInt(offset_y, 10), true);
                });
            ...
            $(posdata).each(function () {
                var pos_x = this.POSX == '' ? 0 : parseInt(this.POSX, 10);
                var pos_y = this.POSY == '' ? 0 : parseInt(this.POSY, 10);
                $('#div_' + this.MACHIDX).offset({ top: parseInt(pos_x, 10) + parseInt(offset_x, 10), left: parseInt(pos_y, 10) + parseInt(offset_y, 10) });

                var param = { "XVal": pos_x, "YVal": pos_y, "MACHIDX": this.MACHIDX, "MACHCD": this.MACHCD, "MACHNM": this.MACHNM, "NO": this.NO, "ID": "div_" + this.MACHIDX };
                $('#div_' + this.MACHIDX).bind('click', function () {
                    dragStop(param, parseInt(pos_x, 10), parseInt(pos_y, 10), false);
        ...
        function dragStart(param, x, y) {
            ...
            $('#tdMach').html(param.MACHNM + '[' + param.MACHCD + ']');
            fn_SetTextValue('hidMACHIDX', param.MACHIDX);
            fn_SetTextValue('txtXPOS', x);
            fn_SetTextValue('txtYPOS', y);
            devGrid.SetFocusedRowIndex(parseInt(param.NO, 10) - 1);
        }

        function dragStop(param, x, y, _isUpdate) {
        ...
        function fn_OnPOSTextChanged(s, e) {
            ...
            $('#div_' + _machIDX).offset({ top: parseInt(_posX, 10) + parseInt(offset_x, 10), left: parseInt(_posY, 10) + parseInt(offset_y, 10) });
        ...
        function fn_OnRowClick(s, e) {
            ...
            $('#tdMach').html(rowKeys[2] + '[' + rowKeys[1] + ']');
            fn_SetTextValue('hidMACHIDX', rowKeys[0]);
            fn_SetTextValue('txtXPOS', rowKeys[3]);
            fn_SetTextValue('txtYPOS', rowKeys[4]);
        }
    </script>
</asp:Content>
    ...
    <div class="container">
        ...
        <div class="content">
            ...
            <dx:ASPxGridView ID="devGrid" ClientInstanceName="devGrid" runat="server" AutoGenerateColumns="false" Width="100%"
                KeyFieldName="F_MACHIDX;F_MACHCD;F_MACHNM;F_POINTX;F_POINTY;NO" EnableViewState="false" EnableRowsCache="false"
                ...
                <Columns>
                    ...
                    <dx:GridViewBandColumn Caption="설비위치">
                        <Columns>
                            <dx:GridViewDataColumn FieldName="F_POINTX" Caption="X" Width="100px" />
                            <dx:GridViewDataColumn FieldName="F_POINTY" Caption="Y" Width="100px" />
                        </Columns>
