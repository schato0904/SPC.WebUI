<%@ Page Title="" Language="C#" MasterPageFile="~/Resources/controls/MasterPage/spcPopupMasterPage.Master" AutoEventWireup="true" CodeBehind="BSIF0303POPNEW.aspx.cs" Inherits="SPC.WebUI.Pages.BSIF.Popup.BSIF0303POPNEW" %>
<asp:Content ID="Content1" ContentPlaceHolderID="cphHead" runat="server">
    <script type="text/javascript">
        ...
        function fn_OnNewClick() {
            ...
            $('#currentStatus').text('입력모드(작성을 완료한 후 반드시 저장을 눌러야 서버에 반영됩니다)');
            fn_SetTextValue('editingMode', 'i');
        }
        ...
        function fn_OnSaveClick() {
            if (fn_GetCastValue('editingMode') == 'w') {
                alert('입력을 눌러 신규입력을 하거나 수정할 검사기준을 선택하여 수정을 한 후 저장하세요!!');
                return false;
            }
            ...
            var keys = fn_GetCastValue('editingMode') == 'i' ? 'INS|' : 'UPD|';
            keys += fn_GetCastValue('txtITEMCD') + '|';
            keys += fn_GetCastSelectedItemText('ddlINSPCD') + '|';
            keys += fn_GetCastValue('txtSERIALNO');

            EditForm.PerformCallback(keys);
        }
        ...
        function fn_OnCancelClick() {
            ...
            $('#currentStatus').text('대기모드(입력 또는 수정을 원하시면 입력버튼 클릭 또는 수정할 검사기준을 더블클릭하세요)');
            fn_SetTextValue('editingMode', 'w');
        }
        ...
        function fn_OnValidate() {
            if (fn_GetCastValue('editingMode') == 'i') {
        ...
        function fn_OnControlsInit() {
            ...
            fn_SetTextValue('editingRowVisibleIndex', '-1');

            // DropDownList Init
            ddlINSPCD.SetSelectedIndex(1);
            ddlQCYCLECD.SetSelectedIndex(1);
            ddlJCYCLECD.SetSelectedIndex(1);
            ddlRANK.SetSelectedIndex(1);
            ddlSINGLECHK.SetSelectedIndex(1);
            ddlUNIT.SetSelectedIndex(1);
            ddlAIRCK.SetSelectedIndex(1);
            ddlPORT.SetSelectedIndex(1);
            ddlGETDATA.SetSelectedIndex(1);
            ddlLOADTF.SetSelectedIndex(1);

            ddlINSPCD.SetSelectedIndex(0);
            ddlQCYCLECD.SetSelectedIndex(0);
            ddlJCYCLECD.SetSelectedIndex(0);
            ddlRANK.SetSelectedIndex(0);
            ddlSINGLECHK.SetSelectedIndex(0);
            ddlUNIT.SetSelectedIndex(0);
            ddlAIRCK.SetSelectedIndex(0);
            ddlPORT.SetSelectedIndex(0);
            ddlGETDATA.SetSelectedIndex(0);
            ddlLOADTF.SetSelectedIndex(0);

            rdoDATAUNIT.SetSelectedIndex(-1);
            rdoACCEPT.SetSelectedIndex(-1);
            rdoHIPISNG.SetSelectedIndex(-1);
            rdoSAMPLECHK.SetSelectedIndex(-1);
            rdoRESULTGUBUN.SetSelectedIndex(-1);

            fn_SetTextValue('txtMAX', null);
            fn_SetTextValue('txtMAXCAR', null);
            fn_SetTextValue('txtMIN', null);
            fn_SetTextValue('txtMINCAR', null);
        }

        // Grid RowDblClick
        function fn_OnRowDblClick(s, e) {
            //devGrid.StartEditRow(e.visibleIndex);
            EditForm.PerformCallback('GET|' + devGrid.GetRowKey(e.visibleIndex));
        }
        ...
        function fn_OnValidateStandard(s, e) {
            if (fn_GetCastValue('editingMode') == 'w') return false;
        ...
        function fn_OnEditFormEndCallback(s, e) {
            if (s.cpResultCode != '') {
                if (s.cpResultCode == 'GET') {
                    fn_SetTextValue('editingMode', 'm');
                    $('#currentStatus').text('수정모드(작성을 완료한 후 반드시 저장을 눌러야 서버에 반영됩니다)');
                } else if (s.cpResultCode == 'UPD' || s.cpResultCode == 'INS') {
                    alert(s.cpResultMsg);
                    fn_OnCancelClick();
                }
                else
...
<asp:Content ID="Content2" ContentPlaceHolderID="cphBody" runat="server">
    <div class="container">
        <div class="search">
            <dx:ASPxTextBox ID="editingMode" ClientInstanceName="editingMode" runat="server" ClientVisible="false" Text="w" />
            <dx:ASPxTextBox ID="editingRowVisibleIndex" ClientInstanceName="editingRowVisibleIndex" runat="server" ClientVisible="false" Text="-1" />
            <dx:ASPxCallbackPanel ID="EditForm" ClientInstanceName="EditForm" runat="server" Width="100%" OnCallback="EditForm_Callback">
                <ClientSideEvents EndCallback="fn_OnEditFormEndCallback" CallbackError="fn_OnCallbackError" />
                <Paddings Padding="0" />
                <PanelCollection>
                    <dx:PanelContent>
                        <div id="divEditForm" class="form-horizontal bg-white fa-border" style="margin-bottom: 10px; border: 1px solid #9F9F9F;">
                            <div class="form-group">
                                <div class="col-sm-12 font-bold text-lg">
                                    현재상태 : <span id="currentStatus">대기모드(입력 또는 수정을 원하시면 입력버튼 클릭 또는 수정할 검사기준을 더블클릭하세요)</span>
                                </div>
                            </div>
                            <div class="form-group">
                                <div class="col-sm-12">
        ...
        <div class="content">
            <section class="panel panel-default" style="height: 100%;">
                ...
                <dx:ASPxGridView ID="devGrid" ClientInstanceName="devGrid" runat="server" AutoGenerateColumns="false" Width="100%"
                    ...
                    OnRowDeleting="devGrid_RowDeleting">
                    <Styles>
