...
namespace SPC.WebUI.API.Common.ByPass
{
    ...
    public class redirect : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (true == bExecute)
            {
                string redirectURL = context.Request.Params["url"].Substring(0, context.Request.Params["url"].IndexOf(","));
                string[] paramURL = redirectURL.Split('/');

                redirectURL = String.Format(redirectURL, GetProgramNM(paramURL[2].Substring(0, paramURL[2].IndexOf("."))));
        ...
        bool doLogin(string id, string pw, string compcd, HttpContext context, out string ErrorMsg)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    response.Cookies[KEY]["CSLOGIN"] = "1";
        ...
        #region 프로그램명을 구한다
        string GetProgramNM(string ProgramCD)
        {
            string errMsg = String.Empty;
            DataSet ds = null;
            ...
            {
                Dictionary<string, string> oParamDic = new Dictionary<string, string>();
                oParamDic.Add("F_PGMID", ProgramCD);
                oParamDic.Add("F_LANGTYPE", "KR");
                ds = biz.SYPGM01_LST(oParamDic, out errMsg);
            }

            return ds.Tables[0].Rows[0]["F_PGMNM"].ToString();
        }
        #endregion
